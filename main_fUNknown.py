        ...
        for K in list_K:
            for d in list_d:
                for n_vars_perturbed in list_n_vars_perturbed:
                    ...
                    data, chosen_foods, list_of_objectives, list_of_constraints, constraints, considered_groups, C_orig, W_orig, A, groups_N, list_bn, dataset = instances
                    ...
                    for L in list_L:
                        for lambd in list_lambd:
                            ...
                            already_done = np.any([('soloutput-fUNKNOWN-'+str(lambd)+'-'+name) in string for string in os.listdir(r"./collecting outputs")])
                            if not already_done:
                                ...
                                for n in range(N):
                                    known_objs[n] = {}
                                    for k in range(K):
                                        known_objs[n][k] = C_orig[k,:]@dataset[n,:]
                                ...
                                print(C.X.round(2))
                                print("WITH INTERCEPTS: ", c0.X.round(2))

                                print("ORIGINAL COSTS: ")
                                print(C_orig.round(2))


                                sims = []
                                for k in range(K):
                                    sims.append( cosine_similarity(np.array(C_orig[k,:]).reshape(1, -1), np.array(C.X[k,:]).reshape(1, -1))[0][0] )
                                mean_sim_obj = np.mean(sims)
                                print("COSINE SIMILARITIES IN OBJS: ", sims, " WITH MEAN=",mean_sim_obj)
                                """
                                #depict clusters
                                if K in [2,3]:
                                ...
                                with open('soloutput-fUNKNOWN-'+str(lambd)+'-'+name+'.pkl', 'wb') as handle:
